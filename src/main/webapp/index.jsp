...
<body>
    ...
    <script>
        ...
        function loadHistoryFromSession() {
            <%
                java.util.List<com.lab.PointResult> history = com.lab.HistoryService.readHistory(session);
                if (history != null && !history.isEmpty()) {
                    int index = 0;
                    for (com.lab.PointResult result : history) {
            %>
                        ...
                        historyPoints.push({
                            ...
                            mode: 'markers',
                            type: 'scatter',
                            marker: { color: <%= result.isSuccess() ? "'green'" : "'red'" %>, size: 10 },
                            name: 'point',
                            r: <%= result.getR() %>
                        });
            <%
        ...
        function setRandomBackground() {
            fetch(contextPath + '/api/random-background')
                .then(response => {
                    if (!response.ok) {
